          ...
          <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            {(contact.apex_score || contact.enrichment_status === "completed") && (
              <div className="flex gap-4">
                {contact.apex_score && <ScoreBadge label="APEX" score={contact.apex_score} color="indigo" />}
                {contact.mdc_score && <ScoreBadge label="MDC" score={contact.mdc_score} color="emerald" />}
                {contact.rss_score && <ScoreBadge label="RSS" score={contact.rss_score} color="amber" />}
              </div>
            )}
            {enrichmentData ? (
              ...
              <div className="bg-gray-800 rounded-lg p-6 text-center">
                <p className="text-gray-400 mb-4">No enrichment data yet. Click Enrich to gather AI-powered insights.</p>
              </div>
            )}
            <div className="text-sm text-gray-500 border-t border-gray-700 pt-4">
              ...
              {contact.enriched_at && <span> · Enriched {new Date(contact.enriched_at).toLocaleDateString()}</span>}
            </div>
          </div>
          <div className="flex gap-3 p-6 border-t border-gray-700">
            <EnrichButton contactId={contact.id} onComplete={onEnrichComplete} variant="modal" />
            <button onClick={onClose} className="flex-1 px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white font-medium rounded-lg transition-colors">
              Close
            </button>
          </div>
        </div>
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div>
        ...
        {isLink ? (
          <a href={value.startsWith("http") ? value : `mailto:${value}`} className="text-indigo-400 hover:underline text-sm" target="_blank" rel="noopener noreferrer">{value}</a>
        ) : (
...
function Section({ title, icon, children }: { title: string; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="flex items-center gap-2 text-sm font-semibold text-gray-400 mb-2">{icon}{title}</h3>
      {children}
    </div>
  );
}
