...
export default function ContactsPage() {
	...
	const fetchContacts = async () => {
		...
		try {
			...
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/api/v3/contacts`,
	...
	return (
		<div className="p-6 bg-slate-900 min-h-screen">
			...
			{error && (
				<div className="mb-6 bg-red-900 text-red-100 p-4 rounded-lg flex justify-between">
					<p>{error}</p>
			...
			<div className="mt-6 grid grid-cols-3 gap-4">
				...
				<div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
					...
					<p className="text-3xl font-bold text-blue-500 mt-2">
						{contacts.length > 0 ? (contacts.filter(c => c.apex_score).reduce((s, c) => s + (c.apex_score || 0), 0) / contacts.length).toFixed(0) : 'N/A'}
					</p>
				</div>
			</div>
