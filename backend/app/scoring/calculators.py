...
def calculate_mdcp_score(contact: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Check if company revenue is in range
    min_revenue = cfg.get("moneyMinRevenue", 1000000)
    max_revenue = cfg.get("moneyMaxRevenue", 100000000)
    
    # Placeholder: assume companies with data score 20/25
    if company:
        money_score = 20
    
    # 2. DECISION-MAKER SCORE (25 points)
    title = contact.get("title", "").lower()
    decision_titles = [t.lower() for t in cfg.get("decisionmakerTitles", ["CEO", "VP", "Director", "President", "Owner"])]
    ...
    if decision_score == 0 and title:
        decision_score = 10  # Has title but not decision-maker
    ...
    if contact.get("enrichment_status") == "completed":
        process_score = 20
    elif contact.get("enrichment_status") == "processing":
        process_score = 10
    ...
    # Determine tier
    if total_score >= thresholds["hotMin"]:
        tier = "hot"
    elif total_score >= thresholds["warmMin"]:
        tier = "warm"
    else:
        tier = "cold"
...
def calculate_bant_score(contact: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 1. BUDGET (25 points)
    bant_data = contact.get("enrichment_data", {}).get("bant", {})
    ...
    if bant_data.get("budget_confirmed"):
        ...
        budget_score = 10  # Has company = potential budget
    ...
    if bant_data.get("need_documented"):
        ...
        need_score = 10  # Enriched = some need assessment possible
...
def calculate_spice_score(contact: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    spice_data = contact.get("enrichment_data", {}).get("spice", {})
    ...
    if spice_data.get("problem_identified"):
        problem_score = 20
    elif contact.get("enrichment_data", {}).get("talking_points"):
        problem_score = 10
