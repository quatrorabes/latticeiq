...
# ============================================================
# SUPABASE CLIENT - Direct initialization
# ============================================================
...
def get_workspace_id(authorization: str = Header(None)) -> UUID:
    ...
    For now, uses a default workspace for development.
    """
    # TODO: Implement proper JWT validation when ready
...
def get_field_accessor(supabase: Client = Depends(get_supabase_client)) -> FieldAccessor:
    """Get FieldAccessor instance."""
    return FieldAccessor(supabase)


def get_icp_matcher(
    supabase: Client = Depends(get_supabase_client),
    field_accessor: FieldAccessor = Depends(get_field_accessor)
) -> ICPMatcher:
    """Get ICPMatcher instance."""
    return ICPMatcher(supabase, field_accessor)


def get_variable_substitutor(
    field_accessor: FieldAccessor = Depends(get_field_accessor)
) -> VariableSubstitutor:
    """Get VariableSubstitutor instance."""
    return VariableSubstitutor(field_accessor)


def get_campaign_builder(
    supabase: Client = Depends(get_supabase_client),
    field_accessor: FieldAccessor = Depends(get_field_accessor),
    icp_matcher: ICPMatcher = Depends(get_icp_matcher),
    substitutor: VariableSubstitutor = Depends(get_variable_substitutor)
) -> CampaignBuilder:
    """Get CampaignBuilder instance."""
...
async def phase2_health_check():
    """Health check for Phase 2B services."""
    return HealthResponse(
        status="ok",
        phase2b_services={
            "field_accessor": "operational",
            "icp_matcher": "operational",
            "variable_substitutor": "operational",
            "campaign_builder": "operational"
        },
...
async def create_icp(
    ...
    """Create a new Ideal Client Profile."""
    try:
        icp_id = uuid4()
        ...
        if scoring_weights is None:
            ...
            scoring_weights_dict = scoring_weights.dict()
        
        response = supabase.table("ideal_client_profiles").insert({
            "id": str(icp_id),
        ...
        logger.info(f"Created ICP {icp_id} for workspace {workspace_id}")
...
async def list_icps(
    ...
    """List all ICPs for the current workspace."""
    try:
        query = (
            supabase.table("ideal_client_profiles")
            .select("*")
...
async def get_icp(
    ...
    try:
        ...
        logger.error(f"Error getting ICP: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_icp(
    ...
    try:
        ...
        logger.info(f"Updated ICP {icp_id}")
        return await get_icp(icp_id, workspace_id, supabase)
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error updating ICP: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_icp(
    ...
    try:
        ...
        }).eq("icp_id", str(icp_id)).execute()
        
        supabase.table("ideal_client_profiles").delete().eq("id", str(icp_id)).eq("workspace_id", str(workspace_id)).execute()
        
        logger.info(f"Deleted ICP {icp_id}")
        return SuccessResponse(success=True, message=f"ICP {icp_id} deleted successfully")
        
    except Exception as e:
        logger.error(f"Error deleting ICP: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/icps/{icp_id}/matches", response_model=MatchingContactsResponse)
async def get_icp_matches(
    icp_id: UUID,
    ...
    """Get contacts matching an ICP above a score threshold."""
    try:
        ...
        icp_name = icp_response.data["name"]
        
        contacts_response = (
            supabase.table("contacts")
            .select("id, first_name, last_name, email, company, job_title, icp_match_score, kernel_who_persona, enrichment_company_industry")
            .eq("workspace_id", str(workspace_id))
        ...
        for row in contacts_response.data:
            contacts.append({
                ...
                "icp_match_score": row.get("icp_match_score", 0),
                "persona": row.get("kernel_who_persona"),
                "industry": row.get("enrichment_company_industry")
            })
        
        return MatchingContactsResponse(
            icp_id=icp_id,
            icp_name=icp_name,
            min_score=min_score,
            contacts=contacts,
        ...
        logger.error(f"Error getting ICP matches: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/icps/{icp_id}/match", response_model=BulkMatchResponse)
async def bulk_match_contacts(
    icp_id: UUID,
...
async def create_campaign(
    ...
    workspace_id: UUID = Depends(get_workspace_id),
    campaign_builder: CampaignBuilder = Depends(get_campaign_builder),
    supabase: Client = Depends(get_supabase_client)
):
    """Create a new campaign targeting an ICP."""
    try:
        campaign_id = campaign_builder.build_campaign(
            workspace_id=workspace_id,
        ...
        return CampaignResponse(
            ...
            workspace_id=UUID(row["workspace_id"]),
            name=row["name"],
            icp_id=UUID(row["icp_id"]),
            email_template_id=UUID(row["email_template_id"]),
            status=CampaignStatus(row["status"]),
            target_count=row.get("target_count", 0),
            sent_count=row.get("sent_count", 0),
            opened_count=row.get("opened_count", 0),
            clicked_count=row.get("clicked_count", 0),
            replied_count=row.get("replied_count", 0),
            open_rate=0.0,
            click_rate=0.0,
            reply_rate=0.0,
...
async def list_campaigns(
    ...
    try:
        ...
        for row in response.data:
            ...
            opened = row.get("opened_count", 0)
            clicked = row.get("clicked_count", 0)
            replied = row.get("replied_count", 0)
...
async def get_campaign_preview(
    ...
    """Get campaign details and preview of personalized emails."""
    try:
        campaign_response = supabase.table("campaigns").select("*, email_templates(name)").eq("id", str(campaign_id)).eq("workspace_id", str(workspace_id)).single().execute()
        ...
        preview_contacts = [
            CampaignPreviewContact(
                contact_id=UUID(p["contact_id"]),
                contact_name=p["contact_name"],
                email=p.get("email"),
                email_subject=p.get("email_subject", ""),
                email_body_preview=p.get("email_body_preview", ""),
                icp_match_score=p.get("icp_match_score", 0)
            )
            for p in previews
        ]
        
        template_name = campaign.get("email_templates", {}).get("name", "Unknown Template") if campaign.get("email_templates") else "Unknown Template"
        
        return CampaignPreviewResponse(
            campaign_id=campaign_id,
            campaign_name=campaign["name"],
            template_name=template_name,
            total_targets=campaign.get("target_count", 0),
            previews=preview_contacts
        )
...
async def activate_campaign(
    ...
    try:
        ...
        success = campaign_builder.activate_campaign(campaign_id)
        ...
        return CampaignActivateResponse(
            ...
            target_count=campaign_response.data["target_count"],
...
async def preview_template(
    ...
    try:
        ...
        if request.template_id:
            ...
            variables_missing = preview["variables_missing"]
        else:
            body = substitutor.substitute(request.template_text, request.contact_id, request.extra_values)
            all_vars = substitutor.extract_variables(request.template_text)
        ...
        contact_name = f"{contact_fields.get('first_name', '')} {contact_fields.get('last_name', '')}".strip()
...
async def get_available_variables(
    ...
    """Get all available variables for a contact."""
    try:
        all_fields = field_accessor.get_all_available_fields(contact_id)
        
        first_name = all_fields.get("first_name", "")
        last_name = all_fields.get("last_name", "")
        contact_name = f"{first_name} {last_name}".strip() or "Unknown"
        
        return AvailableVariablesResponse(
            contact_id=contact_id,
            contact_name=contact_name,
            available_variables=list(all_fields.keys()),
            variable_values=all_fields
        )
...
async def get_field_values(
    ...
    """Get specific field values for a contact."""
    try:
